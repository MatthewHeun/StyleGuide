...
``This,'' ``That,'' or ``It'' as the subject of a sentence
is almost always problematic, because
the antecedent is almost always unclear to the reader.
...
is MKH's pet peeve.
He apologizes in advance for any inconvenience.
...
are reserved words for stages in the energy conversion chain.
...
Writing in \LaTeX{} will make your life easier.
...
Use the prefix ``ll'' for \LaTeX{} commands and ``llg'' for Greek letters.

macOS includes type expansion in the Keyboard area of settings.
Tools such as \href{https://www.alfredapp.com}{Alfred} also
provide a \texttt{{cursor}} macro that indicates the desired
