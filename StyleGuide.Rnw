...
\begin{itemize}
  ...
  \item To include subscripts use the code:
        \$ \texttt{\textbackslash mathrm} kJ/mol \$
...
ctan.org is a website for LaTeX packages that exist.
Under the packages tab
in the terminal of RStudio,
there is an install tab
and a list of available packages.
At the beginning of an RStudio project,
include \texttt{usepackage{package name}}.

For chemical equations,
package mhchem or flutter tex
are useful.
\href{https://ctan.math.illinois.edu/macros/latex/contrib/mhchem/mhchem.pdf}
is a helpful pdf for using mhchem.
