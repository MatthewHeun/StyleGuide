...
Units are not written in math mode.

\begin{itemize}

  \item Dollar signs (\$) are unnecessary to format units,
  ...
  \item When two or more units are multiplied in a numerator or denominator
        use a dot instead of a hyphen. Like this:\\
        $\mathrm{kJ/mol \cdot K}$
        %
        where the dot comes from \texttt{\textbackslash cdot}.
  \item To include subscripts use the code:
...
\begin{itemize}
  ...
  \item Use subsections with headings to provide
        the reader guidance as they read your paper or report.
...
(which does these these things automatically),
follow these rules:

\begin{itemize}

  \item Center equations and put equation numbers at right margin.
        See textbooks for formatting. Do not include the word ``equation.''

  \item Number pages at bottom center,
  ...
  \item Do not give numbers without units!
